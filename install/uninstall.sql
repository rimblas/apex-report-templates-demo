
drop table books purge;
drop table app_todos purge;

drop table app_roles purge;
drop sequence app_roles_seq;

drop table app_email_phone_types purge;
drop sequence app_email_phone_types_seq;

drop table app_client_contacts purge;
drop sequence app_client_contacts_seq;

drop table app_party purge;
drop sequence app_party_seq;

drop table app_contact_roles purge;
drop sequence app_contact_roles_seq;

drop table app_clients purge;
drop sequence app_clients_seq;
drop sequence app_client_no_seq;

